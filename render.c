...
static void draw_particle(EGG_PARTICLE *part, EGG *egg, double *image, int w, int h)
{
	 ...
	 if ((xx >= 0) && (xx < w) && (yy >= 0) && (yy < h)) {
	    ...
	    if (cvg > 0) {
		  ...
		  cvg *= a;
	       ...
	       putpix(image+(xx+yy*w)*3, r, g, b, cvg, add, mul, sub);
   ...
   if ((!drawn) && (!aa)) {
      ...
      if ((xx >= 0) && (xx < w) && (yy >= 0) && (yy < h))
	 putpix(image+(xx+yy*w)*3, r, g, b, a, add, mul, sub);
   }
}
...
{
   EGG_PARTICLE *part, **part_list;
   double *image, *im;
   int i, x, y, bpp;

   image = malloc(sizeof(double)*bmp->w*bmp->h*3);

   for (i=0; i<bmp->w*bmp->h*3; i++)
      image[i] = 0;
   ...
   if (egg->part_count > 0) {
	 ...
	 qsort(part_list, egg->part_count, sizeof(EGG_PARTICLE *), part_cmp);
      ...
      for (i=0; i<egg->part_count; i++)
	 draw_particle(part_list[i], egg, image, bmp->w, bmp->h);
   ...
   for (y=0; y<bmp->h; y++) {
	 ...
	 putpixel(bmp, x, y, makecol_depth(bpp, r, g, b));
